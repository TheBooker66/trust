function PayoffsUI(config) {
    ...
    const numbers = [];
    const _makeIncDec = function (letter, x, y) {
        (function (letter, x, y) {

            const number = new IncDecNumber({
                x: x, y: y, max: 5, min: -5,
    ...
    self.remove = function () {
        unlisten(self);
        for (let i = 0; i < numbers.length; i++) unlisten(numbers[i]);
        for (let i = 0; i < self.slideshow.dom.children.length; i++) {

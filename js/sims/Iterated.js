...
function Iterated(config) {
    ...
    // APP
    const app = new PIXI.Application(700, 250, {transparent: true, resolution: 2});
    self.app = app;
    ...
    // LABELS
    const _l1 = _makeLabel("label_they_cooperate", {
        x: 354,
        y: 34,
        rotation: 45,
        align: "center",
    ...
    const _l2 = _makeLabel("label_you_cooperate", {
        x: 272,
        y: 35,
        rotation: -45,
        align: "center",
    ...
    const _l3 = _makeLabel("label_they_cheat", {
        x: 406,
        y: 86,
        rotation: 45,
        align: "center",
    ...
    const _l4 = _makeLabel("label_you_cheat", {
        x: 224,
        y: 83,
        rotation: -45,
        align: "center",
    ...
    self.oneoffHighlight1 = function (yourAnswer) {
        ...
        const your = yourAnswer === "COOPERATE" ? _l2 : _l4;
    ...
    self.chooseOpponent = function (id) {
        const LogicClass = window["Logic_" + id];
    ...
    self.playOneRound = function (yourMove) {
        ...
        const B = self.opponentLogic.play();

        // Get payoffs
        const payoffs = PD.getPayoffs(A, B);

        // ANIMATE the moves: betrayal or what?
        self.playerA.TRIP = (yourMove === "TRIP");
        const animPromise1 = self.playerA.playMove(payoffs[0]); // reward, temptation, sucker, punishment, etc...
        const animPromise2 = self.playerB.playMove(payoffs[1]);
...
function IteratedPeep(config) {
    ...
    self.payoffCoins = [];
    for (let i = 0; i < 3; i++) {
        let c = _makeMovieClip("iterated_coin", {scale: 0.5});
        c.visible = false;
    ...
    let _isTripping = false;
    let _isHopping = false;
    let _faceTripped = false;
    let _hopTimer = 0;
    let _faceTimer = 0;
    self.update = function (delta) {
        ...
        if (_faceTripped) {
            _faceTimer += 0.25;
            const frame = 18 + (Math.floor(_faceTimer) % 5);
            ...
            Tween_get(self.coin)
                ...
                .call(function () {
                    ...
                    if (self.payoff === PD.PAYOFFS.T) {
                        setTimeout(function () {
                            const stereo = (config.opponent) ? 0.9 : -0.9;
    ...
    self.kill = function () {
        ...
        for (let i = 0; i < self.payoffCoins.length; i++) {

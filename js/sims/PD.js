...
PD.playRepeatedGame = function (playerA, playerB, turns) {
    ...
    for (let i = 0; i < turns; i++) {
        const p = PD.playOneGame(playerA, playerB);
...
PD.playOneTournament = function (agents, turns) {
    ...
    // Round robin!
    for (let i = 0; i < agents.length; i++) {
        const playerA = agents[i];
        for (let j = i + 1; j < agents.length; j++) {
            const playerB = agents[j];
...
function Logic_prober() {
    ...
    const moves = [PD.COOPERATE, PD.CHEAT, PD.COOPERATE, PD.COOPERATE];
    let everCheatedMe = false;
